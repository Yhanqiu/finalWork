...
/**
 * Created by hanqiu on 2017/10/6.
 */
...
public class BackgroundAdapter {
    ...
    public static Bitmap getForegroundBitmap(Context context, int musicPicRes) {
    ...
    public static Drawable getForegroundDrawableFromFile(Context context, String filePath) {
        /*得到屏幕的宽高比，以便按比例切割图片一部分*/
        final float widthHeightSize = (float) (DisplayUtil.getScreenWidth(context)
                * 1.0 / DisplayUtil.getScreenHeight(context) * 1.0);

        Bitmap bitmap = getForegroundBitmapFromFile(context, filePath);
        int cropBitmapWidth = (int) (widthHeightSize * bitmap.getHeight());
        int cropBitmapWidthX = (int) ((bitmap.getWidth() - cropBitmapWidth) / 2.0);
...
//                .getHeight() / 50, false);
        /*模糊化*/
//        final Bitmap blurBitmap = FastBlurUtil.doBlur(scaleBitmap, 8, true);
//        final Bitmap blurBitmap = FastBlurUtil.doBlur(cropBitmap, 8, true);

//        final Drawable foregroundDrawable = new BitmapDrawable(blurBitmap);
        Drawable foregroundDrawable = new BitmapDrawable(cropBitmap);
        /*加入灰色遮罩层，避免图片过亮影响其他控件*/
        foregroundDrawable.setColorFilter(Color.GRAY, PorterDuff.Mode.MULTIPLY);
        return foregroundDrawable;
    }

    public static Bitmap getForegroundBitmapFromFile(Context context, String filePath) {
        int screenWidth = DisplayUtil.getScreenWidth(context);
        int screenHeight = DisplayUtil.getScreenHeight(context);
